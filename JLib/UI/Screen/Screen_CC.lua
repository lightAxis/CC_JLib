...
-- @param text:string
function Screen:write(text) self._screen.write(text) end
...
function Screen:bilt(text, fg, bg) self._screen.bilt(text, fg, bg) end

-- @brief Clears the entire screen.
function Screen:clear() self._screen.clear() end

-- @brief Clears the line the cursor is on.
function Screen:clearLine() self._screen.clearLine() end

-- @brief Returns two arguments containing the x and the y position of the cursor.
-- @return JLib.Vector2
function Screen:getCursorPos()
    local x, y = Screen:getCursorPos_Raw()
